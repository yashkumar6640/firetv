...
(function(exports) {
    ...
    setTimeout(function() {
      if (window.innerWidth < window.innerHeight) {
        $("#overlay-message").html(
          "please rotate your device back to landscpe"
        );
  ...
  function App(settingsParams) {
    ...
    this.handleTouch = function(e) {
      if (e.type === "swipe") {
        if (
          $("#left-nav-list-container").hasClass("leftnav-menulist-collapsed")
        ) {
      ...
      leftNavView.on(
        ...
        function(index) {
          if (!this.showSearch || index !== 0) {
            ...
            //show the spinner
            this.loadingSpinner.show.spinner();
            this.oneDView.updateCategoryFromSearch(
              this.searchInputView.currentSearchQuery
            );
    ...
    this.initializeOneDView = function() {
      ...
      oneDView.on(
        ...
        function(index) {
          ...
          if (this.categoryData[index].type === "subcategory") {
            ...
            this.categoryData[index].type === "video-live" &&
            !this.categoryData[index].isLiveNow
          ) {
      ...
      var successCallback = function(categoryData) {
        ...
        oneDView.render(
          this.$appContainer,
          categoryData,
          this.settingsParams.displayButtons
        );
      }.bind(this);
      ...
      subCategoryView.on(
        ...
        function(index) {
          if (this.subCategoryView.data[index].type === "subcategory") {
            ...
            this.subCategoryView.data[index].type === "video-live" &&
            !this.subCategoryView.data[index].isLiveNow
          ) {
            alert("This video is not yet available.");
            buttons.resync();
          } else {
    ...
    this.updateLiveStream = function(data, index) {
      if (this.playerView) {
        this.playerView.updateTitleAndDescription(
          data[index].title,
    ...
    this.transitionToPlayer = function(data, index) {
      ...
      if (
        this.settingsParams.PlaylistView &&
        data[index].type !== "video-live"
      ) {
        playerView = this.playerView = new this.settingsParams.PlaylistView(
          this.settingsParams
        );
      } else {
        playerView = this.playerView = new this.settingsParams.PlayerView(
          this.settingsParams
        );
      }
      this.oneDView.hide();
      ...
      playerView.on(
        "error",
        function(errType, errStack) {
          var errorDialog;

          switch (errType) {
            case ErrorTypes.PLAYER_ERROR:
              var buttons = this.createOkButtonForErrorDialog(
                this.exitAppCallback
              );
              errorDialog = errorHandler.createErrorDialog(
                errType.errTitle,
                errType.errToUser,
                buttons
              );
            ...
            case ErrorTypes.EMBEDDED_PLAYER_ERROR:
              var buttons = this.createButtonsForErrorDialog(
                this.playerErrorOkCallback,
                this.playerErrorRetryCallback
              );
              errorDialog = errorHandler.createErrorDialog(
                errType.errTitle,
                errType.errToUser,
            ...
            default:
              errType.errToDev =
                "An unknown error occurred in the player adapter";
    ...
    this.playerErrorRetryCallback = function() {
      ...
      if (
        this.appViewBeforeError instanceof PlaylistPlayerView ||
        this.appViewBeforeError instanceof PlayerView
      ) {
        this.transitionFromErrorDialog();
        this.playerView.remove();
      ...
      function(errType, errStack) {
        ...
        switch (errType) {
          ...
          case ErrorTypes.INITIAL_NETWORK_ERROR:
            ...
            buttons = this.createButtonsForErrorDialog(
              this.exitAppCallback,
              this.initialFeedErrorRetryCallback
            );
            errorDialog = errorHandler.createErrorDialog(
              errType.errTitle,
              errType.errToUser,
          ...
          case ErrorTypes.CATEGORY_NETWORK_ERROR:
            buttons = this.createButtonsForErrorDialog(
              this.categoryErrorOkCallback,
              this.categoryErrorRetryCallback
            );
            errorDialog = errorHandler.createErrorDialog(
              errType.errTitle,
              errType.errToUser,
          ...
          case ErrorTypes.SUBCATEGORY_NETWORK_ERROR:
            buttons = this.createButtonsForErrorDialog(
              this.subCategoryErrorOkCallback,
              this.subCategoryErrorRetryCallback
            );
            errorDialog = errorHandler.createErrorDialog(
              errType.errTitle,
              errType.errToUser,
          ...
          case ErrorTypes.SEARCH_NETWORK_ERROR:
            buttons = this.createButtonsForErrorDialog(
              this.searchErrorOkCallback,
              this.searchErrorRetryCallback
            );
            errorDialog = errorHandler.createErrorDialog(
              errType.errTitle,
              errType.errToUser,
          ...
          default:
            ...
            errType.errToDev =
              "An unknown error occurred in the data model adapter";
