  ...
  function PlayerView(settings) {
    ...
    this.timeUpdateHandler = function() {
      ...
      if (!this.videoElement.paused) {
        ...
        this.playerSlowResponse = setTimeout(
          function() {
            this.trigger(
              "error",
              ErrorTypes.SLOW_RESPONSE,
              errorHandler.genStack()
            );
          }.bind(this),
          this.PLAYER_SLOW_RESPONSE
        );
      }
    ...
    this.render = function($container, data, index) {
      ...
      this.videoElement.addEventListener(
        utils.vendorPrefix("fullscreenchange").toLowerCase(),
      ...
      if (direction > 0) {
        ...
        if (this.buttonDowntime < this.videoElement.duration) {
          ...
          if (
            this.buttonDowntime >
            this.videoElement.duration -
              this.videoElement.duration * this.DECELLERATION_PERCENTAGE_MOMENT
          ) {
          ...
          newPosition =
            this.buttonDowntime +
            this.videoElement.duration * this.FAST_SEEK_JUMP_AMOUNT;
        }
